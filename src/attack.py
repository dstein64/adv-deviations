...
def main(argv=sys.argv):
    ...
    parser.add_argument(
        '--num-saved-images', type=int, help='Number of images to save per class for each attack.')
    devices = get_devices()
    ...
    correct_csv_path = os.path.join(args.workspace, 'eval', 'correct.csv')
    eval_correct = np.loadtxt(correct_csv_path, dtype=bool, delimiter=',')
    net.eval()
    for attack in ATTACKS:
        set_seed(args.seed)
        outdir = os.path.join(attack_dir, attack)
        ...
        for batch_idx, (x_batch, y_batch) in enumerate(test_loader):
            ...
            outputs, representations = net(x_adv_batch, include_reprs=True)
        ...
        reprs_dir = os.path.join(outdir, 'representations')
